...
class ADODB_mysqli extends ADOConnection {
	...
	function _connect($argHostname = NULL, 
			  ...
			  $argDatabasename = NULL, $persist=false)
	  {
		...
		*/
		...
		if ($ok) {
		    echo mysqli_set_charset($this->_connectionID, "utf8");
		    exit();
	...
	function GetOne($sql,$inputarr=false)
	{
		...
		if ($rs) {
			/*
			 * чтобы работали prepared запросы
			 */
			if (is_array($sql)) {
				$rs->fields = $rs->_queryID;
				$rs->_stmt = $sql[1];
				if ($rs->_numOfRows < 1) {
					$rs->EOF = 0;
				}
...
//	    $this->_connectionID = $this->mysqli_resolve_link($this->_connectionID);
	...
	global $ADODB_COUNTRECS;
		...
		if (is_array($sql)) {
			...
			//return $ret;
			/*
			 * далее чтобы работали prepared запросы
			 */
			...
			if ($meta = mysqli_stmt_result_metadata($stmt)) {
...
*/
	...
	function _close() 
	{
		...
		(isset($this->_stmt)) ? mysqli_stmt_free_result($this->_stmt) : mysqli_free_result($this->_queryID); 
	  	$this->_queryID = false;	
	}
